...
namespace CulinaryPortal.Persistence.Services
{
    public class AuthenticationService : IAuthenticationService
    {
        ...
        public async Task<UserDto> AuthenticateAsync(LoginDto request)
        {
            ...
            if (user == null)
            {
                throw new Exception("401", new Exception($"User with username '{request.Username}' not found."));
            }
            var result = await _signInManager.CheckPasswordSignInAsync(user, request.Password, false);
            if (!result.Succeeded)
            {
                throw new Exception("401", new Exception($"Password for '{request.Username}' is not valid."));
            }
